...
const TyreCard = ({ logo, brandName, model, size, price, reviews, warranty, type, availability }) => {
    return (
        <div className="border border-gray-300 rounded-lg p-4 w-80 lg:w-64 xl:w-72 shadow-md transform transition-transform duration-300 ease-in-out hover:scale-105">
            <div className="flex justify-between items-center mb-2">
                {/* <Image src={logo} alt={brandName} width={60} height={30} /> */}
                <h1 className='font-bold text-[20px] text-violet-700'>{brandName}</h1>
                <span className="bg-yellow-400 text-xs font-bold py-1 px-2 rounded">{warranty}</span>
            </div>
            <div className='flex items-center justify-between'>
                <div>
                    <h2 className="text-[16px] font-bold lg:whitespace-nowrap lg:text-[14px] xl:text-[16px]">{model}</h2>
